...
def main():
    ...
    if response.candidates[0].content.parts[0].function_call:
        ...
        if not function_call_result.parts[0].function_response.response:
            raise Exception("Something went wrong, try using --verbose to find out what")  # or similar
...
def call_function(function_call_part, verbose=False):
    ...
    # Take the original args and add working_directory
        args = dict(function_call_part.args)
        args["working_directory"] = "./calculator"
    
        result = function_map[function_call_part.name](**args)
    
    # return the success response here
    return types.Content(
    role="tool",
    parts=[
        types.Part.from_function_response(
